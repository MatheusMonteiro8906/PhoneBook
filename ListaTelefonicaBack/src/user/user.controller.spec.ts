...
describe('UserController', () => {
  ...
  describe('findSome', () => {
    it('should return an array of users', async () => {
      ...
      jest.spyOn(userService, 'findSome').mockResolvedValue(usersMock);

      const result = await controller.findSome('1');
    ...
    it('should return an array of all users when no page is provided', async () => {
      ...
      ]; jest.spyOn(userService, 'findAll').mockResolvedValue(usersMock);

      const result = await controller.findSome();
  ...
  describe('findOne', () => {
    it('should return user phone numbers', async () => {
      ...
      await controller.findOne('1', responseMock);
    ...
    it('should return 422 status when there is an error', async () => {
      jest.spyOn(userService, 'findOne').mockRejectedValue(new Error('Some error'));
